...
static bool has_valid_nav_position(const control_messages::Response& response) {
    const double x = response.other_data[0].value;
    const double y = response.other_data[1].value;
    const double z = response.other_data[2].value;
    if (!std::isfinite(x) || !std::isfinite(y) || !std::isfinite(z)) return false;
    constexpr double kPositionEpsilon = 1e-9;
    return std::abs(x) > kPositionEpsilon || std::abs(y) > kPositionEpsilon || std::abs(z) > kPositionEpsilon;
}
...
void ChannelSubscriber::zero() {
    ...
    std::lock_guard<std::mutex> nav_lock(_nav_data_mutex);
    _navData_lcmt = control_messages::Response{};
    _lastNavPosition_lcmt = control_messages::Response{};
    _hasNavPosition = false;
}
...
void ChannelSubscriber::read_handle(const lcm::ReceiveBuffer* rbuf, const std::string& chan) {
    if (chan == ROBOT_SDK_TOPIC_LOWSTATE) {
        ...
        std::lock_guard<std::mutex> nav_lock(_nav_data_mutex);
        control_messages::Response incoming{};
        if (incoming.decode(rbuf->data, 0, rbuf->data_size) < 0) return;
        const bool valid_position = incoming.code == 3001 && has_valid_nav_position(incoming);
        if (valid_position) {
            _lastNavPosition_lcmt = incoming;
            _hasNavPosition = true;
        }
        _navData_lcmt = incoming;
        if (_hasNavPosition && !valid_position) {
            for (int i = 0; i < 7; ++i) {
                _navData_lcmt.other_data[i] = _lastNavPosition_lcmt.other_data[i];
            }
        }
    }
}
